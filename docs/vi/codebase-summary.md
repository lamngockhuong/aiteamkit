...
| `shared/artifact-paths.md` | Cách xác định thư mục docs gốc, artifact rơi vào repository nào khi dự án trải trên nhiều repository, đường dẫn output mặc định theo skill, quy tắc đặt tên `YYMMDD-HHMM`, bảng các loại tài liệu tham chiếu, ba nhóm độ bền và nhóm nào được commit, cách đánh số ADR, khối front matter dùng chung, và nguyên tắc không ghi đè bản ghi đã `APPROVED` |
...
| `shared/project-overrides.md` | Nội dung của `.atk/overrides/<skill>.md` trong dự án đích, vì sao mỗi skill một file thay vì nhiều file, hai mục `## Before` và `## After`, bảy thứ mà phần ghi đè không bao giờ được gỡ, quy tắc chỉ áp dụng phần ghi đè đã được duyệt cùng dòng skill phải in ra khi không áp dụng nó, dòng skill phải in ra khi bỏ qua một chỉ dẫn, và chỗ thư mục này nằm khi dự án trải trên nhiều repository. Vào qua nguyên tắc 7 của `shared/team-roles.md`, nên mọi skill đều tuân theo |
...
| `shared/diagram-conventions.md` | Khi nào một sơ đồ xứng đáng có mặt trong artifact, bốn dạng hình kit vẽ (luồng duyệt, đồ thị phụ thuộc, sequence, chuỗi nhân quả), và các quy tắc giữ cho chúng dễ đọc: chỉ Mermaid, xuống dòng bằng `<br/>` chứ không bằng ký tự xuống dòng thô, gọi vai trò thay vì gọi tên người, hình thoi nào cũng có đủ hai nhánh, chuỗi dài thì vẽ từ trên xuống, không đặt màu nền cứng. `catchup`, `design-doc`, `plan`, `breakdown`, `security` và `incident` trích dẫn |
| `shared/host-capabilities.md` | Skill được dùng những khả năng nào của agent chủ và gọi tên chúng ra sao, nguyên tắc vẫn cấm gọi tên lệnh của một kit khác, cách xử lý trên harness không có khả năng đó, bốn quy tắc của bước dọn mã, phần chính sách chặn số agent review chạy đồng thời trong một vòng, một lượt hỏi được tính thế nào trên harness gửi được nhiều câu hỏi cùng lúc, và khi nào được nêu tên một kết nối tới dịch vụ bên ngoài. `fix`, `implement`, `verify`, `review`, `design-doc` và `init` trích dẫn, `design-sources.md` cũng vậy |
...
| `skills/design-doc/SKILL.md` | Thiết kế | Tài liệu thiết kế có so sánh phương án, kèm ADR; với `--spike`, một cuộc điều tra có giới hạn thời gian, kết thúc bằng một khuyến nghị; với `--challenge`, các phản biện theo từng vai trò ký duyệt trước khi review |
...
| `skills/qa/references/test-case-template.md` | File test case: các cột, ba nhóm `ACCESSING`, `GUI`, `FUNCTION` và thứ mỗi nhóm không bao giờ chứa, danh sách loại test case, ID không bao giờ cấp lại cho ca khác, bảng Sources mà mỗi lần chạy ghi lại, cột `Source`, cách ánh xạ sang mẫu bảng tính của công ty, quy tắc xuất CSV, và lý do năm cột kết quả chạy để trống trong một tài liệu tham chiếu |
