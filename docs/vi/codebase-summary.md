...
| `skills/qa/references/test-case-template.md` | File test case: các cột, ba nhóm `ACCESSING`, `GUI`, `FUNCTION` và thứ mỗi nhóm không bao giờ chứa, danh sách loại test case, ID không bao giờ cấp lại cho ca khác, cột `Source`, cách ánh xạ sang mẫu bảng tính của công ty, quy tắc xuất CSV, và lý do năm cột kết quả chạy để trống trong một tài liệu tham chiếu |
