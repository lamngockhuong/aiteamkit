...
| `skills/qa/references/test-case-template.md` | The cases file: the columns, the three sections `ACCESSING`, `GUI` and `FUNCTION` and what each never holds, the testcase types, the ID that is never given to a second case, the `Source` column, the mapping onto a company spreadsheet form, the CSV export rules, and why the five execution columns stay empty in a reference document |
