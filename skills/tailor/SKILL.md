...
`shared/project-overrides.md`, and for an update that moved `status` back to `IN REVIEW` in step 4,
that the earlier approved version stops applying as well, since the file has one status.
