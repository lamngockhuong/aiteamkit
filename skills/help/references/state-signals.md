...
| 19 | An override file has any `status` other than `APPROVED`, no front matter, or an approver that is a bare `TBD` | `atk:tailor --audit` | Running the skill as if the override applied: until the named approver approves it, the skill runs as shipped and the team may not notice |
