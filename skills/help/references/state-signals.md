...
| 3 | The current branch changes an API, a table, a feature's behaviour, or a component of a screen that has a screen spec, and the reference document for it is not in the diff | `atk:spec --sync` | `atk:git`, which would carry the change without the document it owes, per `shared/spec-docs.md` |
